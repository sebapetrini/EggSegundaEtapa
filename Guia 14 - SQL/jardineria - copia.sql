...
select puesto, nombre, apellido1, apellido2, puesto
from empleado
...
from pago
where DATE_FORMAT(fecha_pago, '%W') = 'monday';
-- &Y, %m, %d, %W.
...
de ventas.*/
select nombre_cliente, concat(nombre,' ',apellido1,' ', apellido2) as nombre_rep
...
from cliente inner join empleado
on codigo_empleado_rep_ventas = codigo_empleado
where codigo_cliente in (select codigo_cliente from pago group by codigo_cliente);
...
from cliente inner join empleado
on codigo_empleado_rep_ventas = codigo_empleado
where codigo_cliente not in (select codigo_cliente from pago group by codigo_cliente);

/*4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes
junto con la ciudad de la oficina a la que pertenece el representante.*/
select c.nombre_cliente, concat(e.nombre,' ',e.apellido1,' ', e.apellido2) as nombre_rep, o.ciudad
