...
*/
select  p.nombre, p.precio, f.nombre  from producto p inner join fabricante f on p.codigo_fabricante = f.codigo where f.nombre like '%Lenovo%';

/*
5. Devuelve una lista de todos los productos del fabricante Crucial que tengan un precio
mayor que $200.
*/
select  p.nombre, p.precio, f.nombre  from producto p inner join fabricante f on p.codigo_fabricante = f.codigo where f.nombre like '%Crucial%' and p.precio > 200;
/*
6. Devuelve un listado con todos los productos de los fabricantes Asus, Hewlett-Packard.
Utilizando el operador IN.
*/
select  p.nombre, p.precio, f.nombre  from producto p inner join fabricante f on p.codigo_fabricante = f.codigo where f.nombre in ('Asus', 'Hewlett-Packard');
/*
...
producto asociado.
*/
select  f.nombre,  p.nombre, p.precio  from fabricante f left join producto p on p.codigo_fabricante = f.codigo where f.codigo not in (select codigo_fabricante from producto);
/*
Subconsultas (En la cláusula WHERE)
Con operadores básicos de comparación
1. Devuelve todos los productos del fabricante Lenovo. (Sin utilizar INNER JOIN).
*/
select f.nombre,  p.nombre, p.precio  from fabricante f, producto p where p.codigo_fabricante = f.codigo and f.nombre like '%Lenovo%';
/*
2. Devuelve todos los datos de los productos que tienen el mismo precio que el producto
más caro del fabricante Lenovo. (Sin utilizar INNER JOIN)..
*/
select * from fabricante f, producto p where p.codigo_fabricante = f.codigo and p.precio = (select p.precio  from fabricante f, producto p where p.codigo_fabricante = f.codigo and f.nombre like '%Lenovo%' order by p.precio desc limit 1);
/*
3. Lista el nombre del producto más caro del fabricante Lenovo.
*/
select p.nombre, p.precio  from fabricante f, producto p where p.codigo_fabricante = f.codigo and f.nombre like '%Lenovo%' order by p.precio desc limit 1;

/*
4. Lista todos los productos del fabricante Asus que tienen un precio superior al precio
medio de todos sus productos.
*/
select p.nombre, p.precio, f.nombre from fabricante f, producto p where p.codigo_fabricante = f.codigo and f.nombre like '%Asus%' and p.precio < (select avg(precio) from producto); 
...
Select f.nombre, count(p.codigo) from fabricante f, producto p where p.codigo_fabricante = f.codigo group by p.codigo_fabricante;
#Cantidad de productos de Lenovo
select count(codigo) from producto where codigo_fabricante = (select codigo from fabricante where nombre like '%Lenovo%') group by codigo_fabricante;
#Resultado Pregunta

Select f.nombre, count(p.codigo) from fabricante f, producto p 
where p.codigo_fabricante = f.codigo group by p.codigo_fabricante 
having count(p.codigo) = (select count(codigo) from producto where codigo_fabricante = (select codigo from fabricante where nombre like '%Lenovo%') group by codigo_fabricante);
