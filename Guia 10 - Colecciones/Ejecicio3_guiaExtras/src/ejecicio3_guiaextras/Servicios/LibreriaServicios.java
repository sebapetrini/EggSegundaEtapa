 ...
 */
package ejecicio3_guiaextras.Servicios;
...
public class LibreriaServicios {
    ...
    public Libreria crearLibro(){
        ...
        ejemplaresPrest=leer.nextInt();
        return new Libreria(titulo,autor,ejemplares,ejemplaresPrest);    
    }
    
    public boolean prestamo(Libreria libro){
        if (libro.getEjemplaresPrest()<libro.getEjemplares()){
            libro.setEjemplaresPrest(libro.getEjemplaresPrest()+1);
            return true;  
        }else {
            return false;
        }   
     ...
     public boolean devolucion(Libreria libro){
        if (libro.getEjemplaresPrest()>0){
            libro.setEjemplaresPrest(libro.getEjemplaresPrest()-1);
            return true;  
        }else {
            return false;
